...
export default function MyzoneOrders() {
    const searchParamsUtil = useSearchParamsUtil();
    const pageType = searchParamsUtil.get('status') as 'all' | 'cancelled';

    // 필터링 로직
    const getFilteredItems = () => {
        const period = searchParamsUtil.get('period') || 'recent-1month';
        const status = searchParamsUtil.get('status') || 'all';
        const itemName = searchParamsUtil.get('itemName') || '';
            ...
            switch (period) {
                ...
                case '2025':
                    ...
                    filteredItems = filteredItems.filter((item) => {
                        ...
                        return orderDate >= filterDate && orderDate <= endDate2025;
                    });
                    break;
                case '2024':
                    ...
                    filteredItems = filteredItems.filter((item) => {
                        const orderDate = new Date(item.orderDate);
                        return orderDate >= filterDate && orderDate <= endDate2024;
                    });
                    break;
                case '2023':
                    filterDate.setFullYear(2023, 0, 1);
                    const endDate2023 = new Date(2023, 11, 31);
                    filteredItems = filteredItems.filter((item) => {
                        const orderDate = new Date(item.orderDate);
                        return orderDate >= filterDate && orderDate <= endDate2023;
                    });
                    break;
                default:
                    break;
        ...
        if (status !== 'all') {
            ...
            if (targetStatus) {
                filteredItems = filteredItems.filter((item) => item.deliveryStatus === targetStatus);
            }
        }

        // 상품명 필터링
        if (itemName.trim()) {
            filteredItems = filteredItems.filter((item) => item.title.toLowerCase().includes(itemName.toLowerCase()));
        }
    ...
    return (
        <QuickMenuContents>
            <section className="poj2-myzone-orders grid grid-cols-1 lg:grid-cols-2 lg:grid-cols-[180px_calc(100%-200px)] gap-4 lg:gap-5">
                {/* 네비게이션 */}
                ...
                <div className="lg:pt-10 pb-15 lg:pb-30">
                    ...
                    <nav className="sticky top-[57px] h-fit z-2 lg:hidden flex items-center bg-white">
                        ...
                        >
                            <span className={`flex items-center justify-center w-full h-full ${pageType === 'all' ? 'font-semibold border-b-2' : 'border-b border-border'}`}>주문/배송</span>
                        </Link>
                        <Link
                            to={'/myzone/orders?status=cancelled'}
                            className="flex-1 flex items-center justify-center h-[50px]"
                        >
                            <span className={`flex items-center justify-center w-full h-full ${pageType === 'cancelled' ? 'font-semibold border-b-2' : 'border-b border-border'}`}>취소/교환/반품</span>
                        </Link>
                    </nav>

                    {/* 상단 타이틀 */}
                    <div className="max-lg:px-4">
                        <h2 className="max-lg:hidden text-2xl font-semibold">{pageType === 'all' ? '주문/배송 조회' : '취소/교환/반품 조회'}</h2>
                        <div className="mt-6 mb-4">
                            <ListFilterBar pageType={pageType} />
                        </div>
                    </div>

                    <div className="max-lg:p-4">
                        <OrderItemDetailList data={filteredItems} />
                        {/* 결과가 없을 때 메시지 */}
                        {filteredItems.length === 0 && (
                            <div className="text-center py-20 text-description">
                                <p>조건에 맞는 주문 내역이 없습니다.</p>
                            </div>
                        )}
                    </div>
                </div>
...
function ListFilterBar({ pageType }: { pageType: 'all' | 'cancelled' }) {
    const searchParamsUtil = useSearchParamsUtil();
    const [searchQuery, setSearchQuery] = useState(searchParamsUtil.get('itemName') || '');

    const handlePeriodChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        searchParamsUtil.set('period', e.target.value);
    };

    const handleStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        searchParamsUtil.set('status', e.target.value);
    };

    const handleSearchSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (searchQuery.trim()) {
            searchParamsUtil.set('itemName', searchQuery.trim());
        } else {
            searchParamsUtil.delete('itemName');
        }
    };
    ...
    return (
        <form
            ...
            className="flex max-lg:flex-wrap items-center gap-2"
        >
            <select
                id="period-filter"
                value={searchParamsUtil.get('period') || 'recent-1month'}
            ...
            <select
                id="status-filter"
                value={searchParamsUtil.get('status') || 'all'}
