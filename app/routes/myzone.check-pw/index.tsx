...
export default function MyzoneCheckPw() {
    ...
    const validate = () => {
        ...
        if (password.length < 8 || password.length > 12) return '비밀번호는 8~12자로 입력해주세요.';

        const hasLetter = /[a-zA-Z]/.test(password);
        const hasNumber = /[0-9]/.test(password);
        const hasSpecialChar = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(password);
        if (!hasLetter || !hasNumber || !hasSpecialChar) return '영문, 숫자, 특수문자를 모두 포함해야 합니다.';
    ...
    return (
        <QuickMenuContents>
            <section className="poj2-check-pw grid grid-cols-1 lg:grid-cols-2 lg:grid-cols-[180px_calc(100%-200px)] gap-4 lg:gap-5">
                {/* 네비게이션 */}
                ...
                <div className="lg:pt-10 pb-15 lg:pb-30">
                    ...
                    <div className="max-lg:px-4 lg:px-0 mt-6 lg:mt-8">
                        <p className="text-sm mb-4 lg:mb-4">홍길동님의 개인정보를 안전하게 보호하기 위해 비밀번호 확인 후 변경할 수 있습니다.</p>
                        ...
                        >
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full px-4 lg:px-5 py-3 text-sm lg:text-base border placeholder-description transition-colors focus:outline-none focus:border-accent ${error ? 'border-discount' : 'border-border'}`}
                                />
                                {error && <p className="mt-1 text-xs lg:text-sm text-discount">{error}</p>}
                            </div>
